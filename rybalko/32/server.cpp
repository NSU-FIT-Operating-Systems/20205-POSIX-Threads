...
#define CLIENT_QUEUE_SIZE (60)
#define POLL_TIMEOUT (10000)
#define POLL_HOST_TIMEOUT (10000)
...
int RunServer(int port) {
    ...
    while (running) {
        ...
        if (poll_fd.revents & POLLIN) {
            ...
            if (client == nullptr) {
                fprintf(stderr, "Error while allocate memory for client\n");
            }
            client->exited = false;
...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        fprintf(stderr,
                "Wrong number of args. Usage: %s <port>\n", argv[0]);
        pthread_exit(nullptr);
    }
    // read params
    int port = ParseInt(argv[1]);
    if (port < 0 || port > 65535) {
        fprintf(stderr, "Wrong port. The port must be in the range [0; 65535]\n");
        return 1;
    }
