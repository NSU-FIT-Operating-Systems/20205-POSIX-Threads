...
void RestoreTerminal() {
    ...
    if (tcsetattr(tty_fd, TCSANOW, &saved_attr) == -1) {
        write(STDOUT_FILENO, "Error while tcsetattr:", 23);
        exit(EXIT_FAILURE);
    }
}
...
int main(int argc, char** argv) {
    if (argc != 2) {
        fprintf(stderr, "Specify exactly 1 arg: url or host:port\n");
        return 1;
    }

    if (!isatty(tty_fd)) {
        perror("Error not a valid terminal:");
        return 1;
    }

    struct termios attr;
    if (tcgetattr(tty_fd, &saved_attr) == -1) {
        perror("Error while tcgetattr:");
        return 1;
    }
    ...
    if (res < 0) {
        printf("Error while PrintHTTPContents:");
        if (res == -1) {
            close(socket_fd);
        }
    }
